...
experiments(miniACC)
```
A sampleMap can be created in order to examine the relationship between the biological units and experimental results.
```{r}
sampleMap(miniACC)
...
metadata(miniACC)
```
Implementing the MultiAssayExperiment constructor function.
```{r}
